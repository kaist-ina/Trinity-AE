...
def prepare_inputs_roco(X, WQ, WK, WV, K_cache, V_cache):
    """Prepare inputs for Relax roco module."""
    W_qkv = torch.cat([WQ, WK, WV], dim=1)
    return [X, W_qkv, K_cache, V_cache]


def create_relax_roco(M: int, N: int, H: int, D: int, P: int):
    """
    ...
    class Relax_Roco:
        ...
        def forward(
            ...
            with R.dataflow():
                ...
                # Roco
                weights_sum = R.sum(weights, axis=1)
                weights_sqr_sum = R.sum(R.multiply(weights, weights), axis=1)
                ...
                R.output(out, weights_sum, weights_sqr_sum)

            return out, weights_sum, weights_sqr_sum
