...
def prepare_inputs_qknorm(X, WQ, WK, WV, K_cache, V_cache):
    """Prepare inputs for Relax qknorm module."""
    W_qkv = torch.cat([WQ, WK, WV], dim=1)
    return [X, W_qkv, K_cache, V_cache]


def create_relax_qknorm(M: int, N: int, H: int, D: int, P: int):
    """
    ...
    class Relax_QKNorm:
        ...
        def forward(
            ...
            with R.dataflow():
                ...
                q_squared = R.multiply(q, q)
                q_variance = R.divide(R.sum(q_squared, axis=-1, keepdims=True), R.const(D, "float16"))
                q_norm = R.divide(q, R.sqrt(q_variance))

                k_squared = R.multiply(k, k)
                k_variance = R.divide(R.sum(k_squared, axis=-1, keepdims=True), R.const(D, "float16"))
                k_norm = R.divide(k, R.sqrt(k_variance))

                # KV Cache update: in-place update at position P
                cache_K = R.call_tir_inplace(
                    cls.update_cache,
                    (cache_K, k_norm, T.int64(P)),
                    inplace_indices=[0],
                ...
                scores = R.matmul(q_norm, k_for_attn)  # [H, M, L]
