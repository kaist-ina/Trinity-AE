...
def prepare_inputs_vanilla(X, WQ, WK, WV, K_cache, V_cache):
    """Prepare inputs for Relax vanilla module."""
    W_qkv = torch.cat([WQ, WK, WV], dim=1)
    return [X, W_qkv, K_cache, V_cache]


def create_relax_vanilla(M: int, N: int, H: int, D: int, P: int):
    """
