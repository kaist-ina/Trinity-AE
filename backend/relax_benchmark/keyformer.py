...
def prepare_inputs_keyformer(X, WQ, WK, WV, noise, K_cache, V_cache):
    """Prepare inputs for Relax keyformer module."""
    W_qkv = torch.cat([WQ, WK, WV], dim=1)
    return [X, W_qkv, noise, K_cache, V_cache]


def create_relax_keyformer(M: int, N: int, H: int, D: int, P: int):
    """
    ...
    class Relax_Keyformer:
        ...
        def forward(
            ...
            with R.dataflow():
                ...
                perturb = R.divide(R.add(scores, noises), R.const(1.5, "float16"))

                # Softmax with perturb
                weights = R.nn.softmax(scores, axis=-1)
                perturb_out = R.nn.softmax(perturb, axis=-1)
                ...
                R.output(out, perturb_out)

            return out, perturb_out

    return Relax_Keyformer
