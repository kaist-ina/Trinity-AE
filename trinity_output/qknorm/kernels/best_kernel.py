...
def kernel_0(
    ...
    for i in range(0, 4096, BLOCK_I):
        ...
        lv3 = ((1 * lv3).to(tl.float16) + tl.dot(temp_0, temp_4).to(tl.float16)).to(tl.float16)
        ...
        lv5 = ((1 * lv5).to(tl.float16) + tl.dot(temp_0, temp_6).to(tl.float16)).to(tl.float16)
...
def kernel_1(
    ...
    lv30 = tl.zeros((1, 16, BLOCK_N), dtype=tl.float32)
    lv31 = tl.zeros((1, 16), dtype=tl.float32)
    lv36 = tl.zeros((1, 16, 128), dtype=tl.float32)
    ...
    lv13 = tl.sum((lv9 * lv9).to(tl.float16), axis=2, dtype=tl.float16)
    lv19 = tl.sum((lv10 * lv10).to(tl.float16), axis=2, dtype=tl.float16)
    lv17 = (lv9 / tl.sqrt((lv13 / 128.0).to(tl.float32))[:, :, None])
    lv23 = (lv10 / tl.sqrt((lv19 / 128.0).to(tl.float32))[:, :, None])
    ...
    tl.store(const_1_ptr + offset_3, lv23)
    ...
    for n in range(0, 1040, BLOCK_N):
        ...
        lv30 = tl.exp(tl.dot(lv17, temp_7.to(tl.float32)).to(tl.float32))
        lv31 = (tl.sum(lv30, axis=2, dtype=tl.float32) + (1 * lv31))
        offset_6 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * const_2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :, None] * const_2_stride1 + (tl.arange(0, 128))[None, None, :] * const_2_stride2
        mask_11 = (n_indices < 1024)[None, :, None]
        temp_8 = tl.load(const_2_ptr + offset_6, mask=mask_11, other=0.0)
        lv36 = (tl.dot(lv30, temp_8.to(tl.float32)) + (lv36 * 1))
    # Skipped empty sloop with dummy body
    lv36 = (lv36 / lv31[:, :, None])
    temp_9 = tl.permute(lv36, (1, 0, 2))
    offset_7 = (tl.arange(0, 16))[:, None] * lv38_stride0 + (j + tl.arange(0, BLOCK_J))[None, :] * lv38_stride1
    mask_12 = (j_indices < 4096)[None, :]
    tl.store(lv38_ptr + offset_7, tl.reshape(temp_9, (16, 128)).to(tl.float16), mask=mask_12)


# Metadata for benchmark.py
TENSOR_PARAMS = ['const_1', 'const_2', 'lv1', 'lv3', 'lv38', 'lv5', 'p_k_proj_weight', 'p_q_proj_weight', 'p_v_proj_weight', 'x']
FP32_TENSOR_PARAMS = []
BLOCK_PARAMS = ['block_i', 'block_j', 'block_n']

def forward(const_1, const_2, lv1, lv3, lv38, lv5, p_k_proj_weight, p_q_proj_weight, p_v_proj_weight, x, block_i=16, block_j=16, block_n=16):
    """
