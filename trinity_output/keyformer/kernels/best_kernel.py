...
def kernel_0(
    ...
    for i in range(0, 4096, BLOCK_I):
        ...
        lv1 = ((lv1 * 1).to(tl.float16) + tl.dot(temp_0, temp_2).to(tl.float16)).to(tl.float16)
        ...
        lv3 = ((lv3 * 1).to(tl.float16) + tl.dot(temp_0, temp_4).to(tl.float16)).to(tl.float16)
        ...
        lv5 = ((lv5 * 1).to(tl.float16) + tl.dot(temp_0, temp_6).to(tl.float16)).to(tl.float16)
...
def kernel_1(
    ...
    const_3_stride2: tl.constexpr,
    lv1_ptr,
    lv1_stride0: tl.constexpr,
    lv1_stride1: tl.constexpr,
    lv20_ptr,
    lv20_stride0: tl.constexpr,
    lv20_stride1: tl.constexpr,
    lv20_stride2: tl.constexpr,
    lv21_ptr,
    lv21_stride0: tl.constexpr,
    lv21_stride1: tl.constexpr,
    lv21_stride2: tl.constexpr,
    lv28_ptr,
    ...
    lv17 = tl.zeros((1, 16, BLOCK_N), dtype=tl.float16)
    lv22 = tl.zeros((1, 16), dtype=tl.float32)
    lv23 = tl.zeros((1, 16), dtype=tl.float32)
    lv25 = tl.zeros((1, 16, BLOCK_N), dtype=tl.float32)
    lv27 = tl.zeros((1, 16, BLOCK_N), dtype=tl.float32)
    ...
    # Sequential loop n from 0 to 1040 with tile size BLOCK_N
    for n in range(0, 1040, BLOCK_N):
        offset_5 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * const_1_stride0 + (n + tl.arange(0, BLOCK_N))[None, :, None] * const_1_stride1 + (tl.arange(0, 128))[None, None, :] * const_1_stride2
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_10 = (n_indices < 1024)[None, :, None]
        temp_6 = tl.load(const_1_ptr + offset_5, mask=mask_10, other=0.0)
        temp_7 = tl.permute(temp_6, (0, 2, 1))
        lv17 = tl.dot(lv9, temp_7).to(tl.float16)
        offset_6 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * lv20_stride0 + (tl.arange(0, 16))[None, :, None] * lv20_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * lv20_stride2
        mask_11 = (n_indices < 1040)[None, None, :]
        tl.store(lv20_ptr + offset_6, tl.exp(lv17.to(tl.float32)), mask=mask_11)
        offset_7 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * const_3_stride0 + (tl.arange(0, 16))[None, :, None] * const_3_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * const_3_stride2
        mask_12 = (n_indices < 1040)[None, None, :]
        temp_8 = tl.load(const_3_ptr + offset_7, mask=mask_12, other=0.0)
        offset_8 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * lv21_stride0 + (tl.arange(0, 16))[None, :, None] * lv21_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * lv21_stride2
        mask_13 = (n_indices < 1040)[None, None, :]
        tl.store(lv21_ptr + offset_8, tl.exp((lv17 + temp_8).to(tl.float32)), mask=mask_13)
        offset_9 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * lv20_stride0 + (tl.arange(0, 16))[None, :, None] * lv20_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * lv20_stride2
        temp_9 = tl.load(lv20_ptr + offset_9, mask=mask_11, other=0.0)
        lv22 = (tl.sum(temp_9, axis=2, dtype=tl.float32) + (lv22 * 1))
        offset_10 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * lv21_stride0 + (tl.arange(0, 16))[None, :, None] * lv21_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * lv21_stride2
        temp_10 = tl.load(lv21_ptr + offset_10, mask=mask_13, other=0.0)
        lv23 = (tl.sum(temp_10, axis=2, dtype=tl.float32) + (lv23 * 1))
    lv22 = lv22 + 0.0
    lv23 = lv23 + 0.0
    # Sequential loop n from 0 to 1040 with tile size BLOCK_N
    for n in range(0, 1040, BLOCK_N):
        offset_11 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * lv20_stride0 + (tl.arange(0, 16))[None, :, None] * lv20_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * lv20_stride2
        n_indices = n + tl.arange(0, BLOCK_N)
        mask_14 = (n_indices < 1040)[None, None, :]
        temp_11 = tl.load(lv20_ptr + offset_11, mask=mask_14, other=0.0)
        lv25 = (temp_11 / lv22[:, :, None])
        offset_12 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * lv21_stride0 + (tl.arange(0, 16))[None, :, None] * lv21_stride1 + (n + tl.arange(0, BLOCK_N))[None, None, :] * lv21_stride2
        mask_15 = (n_indices < 1040)[None, None, :]
        temp_12 = tl.load(lv21_ptr + offset_12, mask=mask_15, other=0.0)
        lv27 = (temp_12 / lv23[:, :, None])
        offset_13 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None] * lv28_stride0 + (n + tl.arange(0, BLOCK_N))[None, :] * lv28_stride1
        mask_16 = (n_indices < 1040)[None, :]
        tl.store(lv28_ptr + offset_13, tl.sum(lv27, axis=1, dtype=tl.float32).to(tl.float16), mask=mask_16)
        offset_14 = (((j // BLOCK_J)+tl.arange(0, 1)))[:, None, None] * const_2_stride0 + (n + tl.arange(0, BLOCK_N))[None, :, None] * const_2_stride1 + (tl.arange(0, 128))[None, None, :] * const_2_stride2
        mask_17 = (n_indices < 1024)[None, :, None]
        temp_13 = tl.load(const_2_ptr + offset_14, mask=mask_17, other=0.0)
        lv31 = (tl.dot(lv25, temp_13.to(tl.float32)) + (lv31 * 1))
    temp_14 = tl.permute(lv31, (1, 0, 2))
    offset_15 = (tl.arange(0, 16))[:, None] * lv33_stride0 + (j + tl.arange(0, BLOCK_J))[None, :] * lv33_stride1
    mask_18 = (j_indices < 4096)[None, :]
    tl.store(lv33_ptr + offset_15, tl.reshape(temp_14, (16, 128)).to(tl.float16), mask=mask_18)


# Metadata for benchmark.py
TENSOR_PARAMS = ['const_1', 'const_2', 'const_3', 'lv1', 'lv20', 'lv21', 'lv28', 'lv3', 'lv33', 'lv5', 'p_k_proj_weight', 'p_q_proj_weight', 'p_v_proj_weight', 'x']
FP32_TENSOR_PARAMS = ['lv20', 'lv21']
BLOCK_PARAMS = ['block_i', 'block_j', 'block_n']

def forward(const_1, const_2, const_3, lv1, lv20, lv21, lv28, lv3, lv33, lv5, p_k_proj_weight, p_q_proj_weight, p_v_proj_weight, x, block_i=16, block_j=16, block_n=16):
    """
